...
const IntelligenceRecap = () => {
  ...
  useEffect(() => {
    const generateRecap = async () => {
      ...
      if (history.length < 3) return; // Only recap if there's significant history

      // Check if we already showed a recap this session
      const lastRecapTime = sessionStorage.getItem('last_intelligence_recap');
      ...
      try {
        ...
        if (res.ok) {
          ...
          setRecap(data.recap);
          setIsVisible(true);
          sessionStorage.setItem('last_intelligence_recap', now.toString());
          
          // Tactical Delay for TTS to feel "live"
          setTimeout(() => {
      ...
      <div className="bg-slate-900/90 backdrop-blur-3xl border border-blue-500/30 rounded-[2.5rem] p-8 shadow-[0_0_50px_rgba(37,99,235,0.2)] relative overflow-hidden group">
        ...
        <div className="flex justify-between items-start mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xs font-black uppercase tracking-[0.3em] text-blue-400">Tactical Briefing</h3>
              <p className="text-[8px] font-mono text-slate-500 uppercase">Jamie Intelligence Core // Session_Restore</p>
            </div>
          </div>
          <button 
...
export default IntelligenceRecap;
