...
const BookingForm: React.FC<BookingFormProps> = ({ property }) => {
  ...
  const handleBooking = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setBooked(true);
        toast.success('Reservation Deployment Successful.');
      } else {
        const error = await res.json();
        toast.error(error.message || 'Booking failed.');
      }
    } catch (err) {
      toast.error('Network failure in reservation grid.');
    } finally {
      setIsBooking(false);
    }
  };
  ...
  if (booked) {
    return (
      <div className="bg-green-500/10 border border-green-500/30 p-8 rounded-2xl text-center animate-in zoom-in-95 duration-500">
        ...
        <p className="text-slate-400 text-sm mt-2 font-medium">Intelligence locked. Check your email for deployment orders.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-slate-900 border border-white/10 p-8 rounded-2xl shadow-2xl transition-all duration-500 hover:border-blue-500/30">
      <div className="flex items-center gap-3 mb-8 text-blue-400">
        ...
        <h3 className="text-xl font-black uppercase tracking-[0.2em] italic border-l-4 border-blue-500 pl-4">Deployment Grid</h3>
      </div>
      ...
      <form onSubmit={handleBooking} className="space-y-6">
        ...
        <div>
          <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-2 italic">Intelligence Load (Guests)</label>
        ...
        {numNights > 0 && (
          <div className="bg-slate-950/50 border border-white/5 p-6 rounded-2xl animate-in fade-in slide-in-from-top-2">
            ...
            <div className="flex justify-between items-end">
              <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest italic">Total Est. Intelligence Cost</span>
        ...
        {isRV && property.rv_length && (
          <div className="flex items-start gap-3 p-4 bg-orange-500/5 border border-orange-500/20 rounded-xl">
            ...
            <p className="text-[10px] text-orange-200/70 font-medium leading-relaxed uppercase tracking-wider">
              Verification: This spot accommodates rigs up to <span className="text-orange-400 font-bold">{property.rv_length}ft</span>. Ensure your asset matches these specifications before deployment.
            </p>
          </div>
        )}
        ...
        >
          {isBooking ? 'Locking In...' : (
            <>
              <FaCreditCard size={14} /> Initialize Reservation
            </>
          )}
        </button>
      </form>
