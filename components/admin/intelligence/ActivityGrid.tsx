...
const ActivityGrid: React.FC<ActivityGridProps> = ({ bookings }) => {
  return (
    <div className='space-y-4'>
      {bookings.length > 0 ? bookings.map((booking) => (
        ...
        <div className='text-center py-10 opacity-30 text-xs font-black uppercase tracking-widest text-slate-500'>No Active Bookings</div>
      )}
    </div>
  );
};
