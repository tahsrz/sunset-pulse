...
const JamieSprintGenerator = ({ onSprintCreated }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast.success("Roadmap injected into the grid.");
      if (onSprintCreated) onSprintCreated(result.sprint_id);
      setGoal('');
    } catch (err) {
      console.error(err);
      toast.error("Sprint generation sequence failed.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className='bg-purple-500/5 border border-purple-500/20 p-6 rounded-2xl relative overflow-hidden group'>
      ...
      <div className='relative z-10'>
        <div className='flex items-center gap-2 mb-4'>
          ...
          <h2 className='text-xs font-black uppercase tracking-[0.2em] text-white'>Workflow Injection</h2>
        </div>
        
        <textarea
          value={goal}
          onChange={(e) => setGoal(e.target.value)}
          placeholder="Command Jamie: 'Find a rental for Chris' or 'Sweep the North Corridor'..."
