...
export async function resizeBase64Image(
  ...
  const outputMimetype = mimetype === "image/jpg" ? "image/jpeg" : mimetype;
  ...
  const newBuffer = await image.getBuffer(outputMimetype as "image/png" | "image/jpeg");

  return `data:${outputMimetype};base64,${newBuffer.toString("base64")}`;
}
