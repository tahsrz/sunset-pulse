...
Open **Browse all app paths** in [Agent Console](https://www.sunsetpulse.app/agent) or
[Command Center](https://www.sunsetpulse.app/command-center). The global **Ctrl/Cmd+K**
menu searches the same catalog by feature, path, section, and access note.

This source inventory contains **108 page route patterns and 5 non-API handlers**.
It is not a production availability check. APIs under `/api/*`, static assets, and framework
metadata endpoints are not user-navigation commands. Parallel slots under `/jamie-vibes`
and the intercepted TAH modal reuse their parent/canonical URLs; they are not extra paths.

Public entry means the screen can be opened, not that every action is anonymous. Account,
operator, realtor, administrative, and staff/PIN requirements remain enforced by existing
routes and APIs. CMS WIP access follows `VIBE_CMS_PUBLIC_WRITE_WIP`; this navigation change
does not alter it. Administrative screens require sign-in in production (scheduling has
its existing exception); individual actions may impose additional checks.

Paths containing `[id]`, `[slug]`, `[token]`, or `[vibeId]` need actual records. The console
opens a selection workflow when available, and otherwise displays **Context required**.
Checkout returns, signing URLs, and tenant booking links must come from their real workflow.
No fake IDs, checkout sessions, tokens, or customer-site links are generated.
...
Catalog: `apps/pulse/lib/navigation/routeCatalog.ts`. A unit test compares it against page
files and non-API handlers and verifies README coverage, so adding or removing a route
requires updating both the console catalog and this inventory. Directory links disable
prefetching to avoid loading every destination merely by browsing this index.
