...
**Next action: validate capability payloads** — add a server-only admission adapter that resolves the reviewed connector and exact input/output snapshots, validates payloads against the pinned schema hashes, and consumes the existing policy/reservation functions. Keep MCP/OpenAPI dispatch, credentials, external effects and provider activation disabled until payload-validation and effect-gate tests exist. Conditions are available only as bounded predicates over persisted answers; do not add executable expressions or treat a checkpoint as a delivery receipt. Dates are target work slots, not permission to bypass a failed prerequisite.
