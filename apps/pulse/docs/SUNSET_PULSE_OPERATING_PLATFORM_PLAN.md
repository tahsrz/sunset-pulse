...
**Next action: transactional quotas and cost ledgers** — inventory the existing admission reservation model, then add workspace concurrency plus per-run step, cost and time limits before any paid call can be enabled. Keep provider dispatch and production activation disabled while testing quota races and release/reconciliation of abandoned reservations. Conditions remain bounded predicates over persisted answers; do not add executable expressions or treat a checkpoint as a delivery receipt. Dates are target work slots, not permission to bypass a failed prerequisite.
