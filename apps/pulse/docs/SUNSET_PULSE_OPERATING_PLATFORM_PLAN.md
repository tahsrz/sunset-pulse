...
September 22 safety slice: `lib/platform/contracts/capabilityPolicy.ts` now defines bounded connection/tool/schema-hash metadata and immutable effect-receipt fields for future policy review. The inert policy rejects external effects, duplicate capability identities, unallowlisted connections and executable fields; it does not dispatch tools or make nonempty manifests executable. `market-digest.v1.json` now proves a third app can validate through the same manifest schema and scalar form contract without a vertical TypeScript package. `lib/platform/contracts/condition.ts` defines a bounded, non-executable predicate contract, but condition nodes are not yet admitted to run definitions or SQL workers until the coordinated forward migration and branch-interpreter acceptance is implemented.
...
**Next action: begin the next Declarative Engine safety slice** — define the inert capability-policy/effect-receipt contracts and rejection tests before any nonempty manifest capabilities, condition nodes, MCP/OpenAPI dispatch or provider execution. Then prove a third manifest can be validated without a vertical TypeScript package. Do not enable production admissions/providers or treat a checkpoint as a delivery receipt. Dates are target work slots, not permission to bypass a failed prerequisite.
