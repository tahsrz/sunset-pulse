...
September 22 safety slice: `lib/platform/contracts/capabilityPolicy.ts` now defines bounded connection/tool/schema-hash metadata and immutable effect-receipt fields for future policy review. The inert policy rejects external effects, duplicate capability identities, unallowlisted connections and executable fields; it does not dispatch tools or make nonempty manifests executable. `market-digest.v1.json` proves a third app can validate through the same manifest schema and scalar form contract without a vertical TypeScript package. The bounded condition contract is now admitted end-to-end: TypeScript graph validation, forward SQL validation, deterministic `answers.*` evaluation, branch reachability checks, queued generation transitions and app-manifest admission all agree. Conditions remain non-executable predicates; capability nodes, MCP/OpenAPI dispatch and provider effects remain disabled.
...
| Declarative Engine | Manifest contract, revision-checked install API, three reviewed intake fixtures, server-authoritative app launch, inert capability/receipt contracts and bounded condition branching implemented locally. | Resource revisions are pinned; Vibe/content authority remains unavailable. No gateway, capability execution or external effects; manifests still require empty capabilities. |
...
**Next action: begin capability admission safety** — add policy-backed capability declarations, effect-receipt persistence and cost reservations before any nonempty manifest capabilities or MCP/OpenAPI dispatch. Conditions are now available only as bounded predicates over persisted answers; do not add executable expressions, provider calls or external effects. Do not enable production admissions/providers or treat a checkpoint as a delivery receipt. Dates are target work slots, not permission to bypass a failed prerequisite.
