...
**Next action: add health freshness controls to the shared inbox** — add summary counts and bounded pagination for health signals, then prove the combined checkpoint/health read model remains workspace-scoped under concurrent refresh and retry. Keep live provider probes, credentials and production activation disabled until explicit operational review. Conditions remain bounded predicates over persisted answers; do not add executable expressions or treat a checkpoint as a delivery receipt. Dates are target work slots, not permission to bypass a failed prerequisite.
