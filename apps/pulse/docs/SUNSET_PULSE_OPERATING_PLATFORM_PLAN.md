...
**Next action: define connector/schema snapshots** — persist reviewed connector definitions and immutable input/output schema snapshots, then add a server-only admission adapter that consumes the existing policy and reservation functions. Keep MCP/OpenAPI dispatch, credentials, external effects and provider activation disabled until snapshot validation and effect-gate tests exist. Conditions are available only as bounded predicates over persisted answers; do not add executable expressions or treat a checkpoint as a delivery receipt. Dates are target work slots, not permission to bypass a failed prerequisite.
