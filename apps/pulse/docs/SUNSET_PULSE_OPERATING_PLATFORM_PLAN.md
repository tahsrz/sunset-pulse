...
**Next action: add health freshness controls to the shared inbox** — show whether a connector check is due, queued, running or overdue; add bounded operator retry using a new event key while preserving the prior evidence; and prove pagination/inbox freshness without dispatching providers. Keep live provider probes, credentials and production activation disabled until explicit operational review. Conditions remain bounded predicates over persisted answers; do not add executable expressions or treat a checkpoint as a delivery receipt. Dates are target work slots, not permission to bypass a failed prerequisite.
