...
**Next action: add connector health signals** — persist reviewed connector health/check timestamps and surface unavailable or drifted connections in the shared inbox without dispatching providers. Keep real provider calls, credentials and production activation disabled until health tests and operational review exist. Conditions are available only as bounded predicates over persisted answers; do not add executable expressions or treat a checkpoint as a delivery receipt. Dates are target work slots, not permission to bypass a failed prerequisite.
