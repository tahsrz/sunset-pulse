...
**Next action: add operator-controlled health scheduling** — expose a workspace-authorized, disabled-by-default schedule/event control with explicit connector scope, then prove duplicate dispatch, lease recovery and inbox freshness through scheduler acceptance. Keep live provider probes, credentials, retries and production activation disabled until explicit operational review. Conditions remain bounded predicates over persisted answers; do not add executable expressions or treat a checkpoint as a delivery receipt. Dates are target work slots, not permission to bypass a failed prerequisite.
