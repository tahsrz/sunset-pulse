...
**Next action: define the protocol-neutral gateway boundary** — add deterministic MCP/OpenAPI request/response envelopes and fixture-only adapters behind the existing server-only admission path. The gateway must require a validated payload, pinned input/output hashes, a live reservation and an effect checkpoint for any external action; keep real provider calls, credentials and production activation disabled. Conditions are available only as bounded predicates over persisted answers; do not add executable expressions or treat a checkpoint as a delivery receipt. Dates are target work slots, not permission to bypass a failed prerequisite.
