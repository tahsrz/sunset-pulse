...
- [ ] Select the actual non-production app environment first. Use its signed-in `/workspaces` hub to create or select the intended team workspace; workspace creation does not invite participants. Arrange real owner/member/reviewer participation through the approved access process, then pass that exact workspace ID to the preflight. Never create or select a workspace on production for this rehearsal.
- [x] Local `node scripts/platform-local-auth-acceptance.mjs --stack xlyfhiafactxahhvikyv --apply-local-migrations` passed on 2026-09-24 with mock auth disabled: owner-created member/reviewer links, wrong-account denial, explicit acceptance, same-tab second invitation, active-member revocation and subsequent 404, both inert app workflows, canvas/command paths, cancel, pagination, archived denial, foreign-user denial and direct JWT RLS. Reviewed migration `20260924170000_platform_workspace_member_email_cast.sql` was applied only to the local stack. Temporary accounts/workspace were removed and the prior admission flag restored; post-run read-only state was migration `20260924170000`, `platform_run` admission `false`, and zero active team workspaces. This is technical auth/role evidence only, not participant-pilot evidence. Continue reviewing any migration the harness reports missing before passing `--apply-local-migrations`.
...
- Local authenticated browser harness: passed on 2026-09-24 with temporary local Supabase accounts, including rendered invitation create/accept and membership revoke UI, wrong-email denial, and subsequent revoked-member API denial; output is technical auth/role evidence, not participant acceptance.
- Local operational readiness: after harness cleanup, read-only inspection showed migration `20260924170000`, admission disabled, and zero active team workspaces. No workspace-specific preflight or participant run was attempted; do not create/select generated local identities as pilot participants.
- Human access setup UI: `/workspaces` is the authenticated workspace entry; owners/admins can open `/workspaces/{id}/access` to view membership and invitations, create one-time role-scoped email-bound links, copy them only on explicit action, revoke pending invitations after confirmation, and revoke eligible active memberships after confirmation. The UI hides owner revocation and prevents an admin from attempting to revoke another admin; the server/database remains authoritative. Delivery is explicitly `not_sent`; there is no email integration. The bearer token is carried in the URL fragment, moved into tab-scoped session storage and removed from the address bar on page load or a subsequent same-tab invite; it is never included in the page request URL. The acceptance page is no-index/no-referrer, asks the invitee to sign in with the invited email and requires an explicit confirmation before calling the existing server acceptance endpoint. The server remains authoritative for workspace role, invitation expiry/revocation and email matching. Do not place live tokens in logs, tickets or evidence records; issue real invitations only in the selected non-production workspace to intended participants.
