...
- Added the `test:db` package script and a `scheduler-db` CI job that starts only the Supabase database containers, runs `npm run test:db`, and stops the stack even after failure. A local `supabase test db` attempt reached the runner but returned `LegacyDbConnectError` because the Docker-backed Postgres container is not running in this workspace.
