...
**Execution handoff, September 18:** the [September 21–25 platform queue](PLATFORM_WEEK_2026-09-21.md) now determines shared-engine priorities. The September 16 sections below remain domain specifications and dated evidence, not the next week's competing backlog. See the [current ledger](SUNSET_PULSE_OPERATING_PLATFORM_PLAN.md#current-ledger-and-next-session) and [current-head verification record](PLATFORM_VERIFICATION_2026-09-18.md) before relying on older pending-CI statements. Scan reconstruction and external effects remain separately gated.

Platform roadmap (September 18): [Sunset Pulse operating platform — three-phase implementation plan](SUNSET_PULSE_OPERATING_PLATFORM_PLAN.md) now owns Core Platform, Declarative Engine and Control Layer work. JSON run state, unified checkpoints, stored app manifests and MCP/OpenAPI tools replace the former OP00–OP17 architecture. Use its current-action ledger; retain this plan's scheduler, email, capture/privacy and acceptance requirements. The previous platform plan is archived for evidence only.

Status: implementation in progress. The September 16 review below supersedes earlier completion claims and execution priorities, including section 13.7. Docker acceptance now runs in disposable Postgres/Mongo containers; the S6 deferral core passes isolated acceptance, and S5 now has atomic reservation, signed-capability issuance, finalize-time byte/signature/hash validation, bounded object cleanup, and Scan Studio direct-upload progress/retry/cancel locally. Fresh local Supabase/Auth/Storage acceptance and direct signed-upload browser acceptance now pass; remote CI/PR replay and deployment evidence remain open. Property capture remains private intake/review, with no real reconstruction processor enabled.
...
- Current review: production build passes after the deferred scheduler and upload-integrity changes; the full unit suite passes 282 files / 1,122 tests, scheduler Docker acceptance passes 9 groups, and Mongo acceptance passes 9 tests. Focused integrity/versioning/scheduler checks pass 8/8. Fresh local Supabase/Auth/Storage replay and real signed-in browser/storage acceptance now pass the owner upload, private-object denial, foreign-reservation denial and content-provenance path. Changed-object rejection, cleanup retry, remote CI, media decoding and publication remain unverified. Nothing in the local harness proves email delivery or agent execution.
...
Next action: complete the remaining S5 evidence cases (changed-object rejection and cleanup retry), then wire the tested S7 reconciler to the existing scheduler event RPC only after the reconstruction event contract/worker is enabled. The local browser acceptance used a real disposable Supabase Auth user and actual private Storage; it does not establish remote CI, production policy, deployment limits or processor execution. The direct-upload progress/retry UI is implemented locally. No subagents are needed for these packets.
...
| S5 private upload pipeline | Reservation plus direct signed capability, owner-scoped deterministic paths, finalize-time size/MIME/signature validation, SHA-256 provenance, rejected-object cleanup, bounded expired-object reconciliation, direct-upload progress/retry/cancel UI, and local/unit/Mongo/build acceptance; fresh local Supabase/Auth/Storage browser acceptance now passes | Changed-object rejection, cleanup retry, remote CI/migration replay, deployment limits and full media acceptance |
...
| S7 cross-store job bridge | Deterministic operation/event identity, frozen reconstruction payload, bounded idempotent Mongo enqueue intent, approval invalidation fence, and injected retry-safe reconciler implemented with focused tests; production event contract/result projection not enabled | Register the reconstruction event contract only with a worker boundary, then wire the reconciler to idempotent Postgres enqueue and add result projection |
...
Next implementation action: finish changed-object rejection/cleanup-retry evidence, then add the reconstruction event contract and worker boundary before wiring `reconcilePropertyScanReconstructionIntents` to the existing idempotent event RPC. The direct-upload progress/retry/cancel UI is implemented locally. The owner-scoped reviewer assignment feature is implemented locally: `PATCH /api/property-scans/[scanId]/reviewers` replaces assignments behind the current session revision, and Scan Studio can grant or revoke reviewer user IDs. The sprint workspace now presents conservative assignment states, exposes owner-controlled completion for assigned/in-progress work, preserves backlog descriptions/estimates on partial edits, permits revision-fenced edits to proposed sprint items with supported worker selection, keeps a proposed-item draft open after a failed or stale save, uses accurate Save schedule copy for paused schedules, the shortlist planner displays created-task counts plus actionable exclusions while labeling notes as You or Jamie, and Pulse command results can be added once to the owner backlog with command provenance. S6 now adds replay-safe event jobs and a bounded sprint-planner event producer contract; it does not register property reconstruction or auto-send. The prior supported full-suite evidence remains 282 files / 1,122 tests; this turn's root-invocation suite was path-invalid and reported 275 passing files / 1,123 tests with 8 environment/fixture-sensitive failures, so it is not a replacement acceptance count. New S7 focused tests pass 10/10; the production build passes with only the existing non-fatal Kepler dynamic-usage warning. Local real auth/storage acceptance passes the happy path plus anonymous/foreign isolation and content provenance; changed-object rejection, cleanup retry, CI migration replay, S7 event contract/worker, result projection, S8 processor and S9 publication remain gated. Do not spend the next sprint polishing the room-shell demo.
...
| Last accepted Luna packet | S5 local direct/private upload integrity, bounded cleanup, Scan Studio direct-upload UI, local real Supabase/Auth/Storage browser happy-path/isolation evidence, and S7 deterministic reconstruction identity plus bounded Mongo intent persistence |
| Active / next packet | S5 changed-object/cleanup-retry evidence, then S7's bounded scheduler reconciliation worker. Recurring schedules and one-off events already share durable jobs, leases, retries and owner fencing |
...
| Remaining blockers | Changed-object rejection and cleanup retry against real Storage; remote CI/PR migration replay; deployment limits; direct-upload progress/retry evidence outside the local browser run; reconstruction event contract/worker and result projection; actual processor unselected; no commit/push/deployment/remote migration in this turn |
| Changed paths / commit | Runtime, tests and plan docs changed locally; not committed |
| Exact next action | Add the bounded S5 changed-object/cleanup-retry acceptance, then define the reconstruction event contract and worker boundary before wiring the tested S7 reconciler to `enqueue_workflow_event` |
