...
Status: implementation in progress. The September 16 review below supersedes earlier completion claims and execution priorities, including section 13.7. Docker acceptance now runs in disposable Postgres/Mongo containers; S6 is still incomplete because asynchronous deferral and several race/permission cases remain open. Property capture remains private intake/review, with no real reconstruction processor enabled.
...
- Local `npm run docker:test`: scheduler assertions passed (observed concurrent scheduled/event claims, observed duplicate-insert contention, omitted-time replay, owner separation, changed-payload/time rejection, expired-token rejection before recovery, live completion receipt and denied browser enqueue); 7 real Mongo integration tests passed. New forward repair also reapplied successfully in the isolated fixture.
...
- Review unit/typecheck results are recorded at handoff below. Full Supabase replay of the new forward migration, real signed-in browser/storage acceptance, remote CI, media decoding and publication remain unverified. Nothing in the local harness proves email delivery or agent execution.
...
| --- | --- | --- |
| 0:00–0:50 | 1. `workflowRegistry.server.ts`: define terminal/deferred result union with validated next-poll time. 2. New forward migration: fenced `defer_workflow_job`, preserving failure budget and bounding polls/deadline. 3. `durableScheduler.server.ts`: dispatch the union through complete/defer RPCs. 4. Extend isolated scheduler fixture with expiration during lock wait, pause/claim and cancel/defer races. | Deferred work cannot exhaust failure attempts or revive cancelled jobs; existing sprint/email terminal handlers unchanged in behavior. S6 remains open until these cases pass. |
| 0:50–1:50 | 1. `PropertyScanSession.ts`: bounded upload reservations with upload ID, idempotency key, declared bytes/type, expected revision, expiry and state. 2. New `lib/scans/scanUpload.server.ts`: atomic reserve/finalize/abort/reconcile operations. 3. Count assets plus all outstanding reservations within the same Mongo conditional write. | Initial pilot: max 24 assets, max 300 MiB/session, max 75 MiB/file, two active uploads/session. Server-owned constants; no caller-controlled quota. Lost finalize responses replay the same asset. Test parallel quota contention in Docker Mongo. |
| 1:50–2:50 | 1. New `.../[scanId]/uploads/route.ts`: owner/consent/revision validation and exact-path signed upload capability. 2. New `.../uploads/[uploadId]/complete/route.ts`: inspect actual object size, checksum and decode support before attaching. 3. Keep unverified uploads separate from finalized immutable objects. 4. Reject unsupported formats explicitly. | Large bytes bypass the Next request handler. A still-live upload capability cannot mutate a finalized/gallery asset; pending objects never receive gallery links. No claim of native LiDAR support. |
| 2:50–3:35 | 1. Audit Supabase `storage.objects` policies in an isolated Supabase stack. 2. Add a forward restrictive bucket policy where needed. 3. Add Docker storage acceptance for foreign/anonymous read/write/list denial, changed object rejection and failed cleanup retry. 4. Wire bounded reconcile into the existing scheduler tick only. | Retry/orphan behavior has durable receipts; S5 cannot close from mocked upload success. Do not launch a second application scheduler or use live media. |
| 3:35–4:20 | 1. `ScanStudioWorkspace`/current `scan-studio/page.tsx`: direct upload progress, retry/cancel and per-file outcomes. 2. Add real owner/reviewer browser fixtures with local Supabase identities, including revocation and stale review rejection. 3. Add SprintCard refresh/conflict behavior to the signed-in sprint flow. | A user can recover a failed upload or edit with Jamie context intact. Keep E2E bypass tests separate from actual authorization evidence. |
| 4:20–5:00 | Run affected Docker, unit and browser checks; replay migrations on fresh Supabase and upgrade fixtures; update this ledger and Keller/Westlake handoff with exact evidence. Investigate profile-trigger signup failure before claiming real auth acceptance. | Reviewable code and an honest release status. CI must run on the eventual PR revision; local passes are not CI evidence. |
...
Final review checks: 31 focused unit tests passed across seven files, including refreshed-draft revision fencing and command-backlog insert contention. The full TypeScript check still fails in existing test fixtures outside this slice; no production build or remote CI pass is claimed for this review. Development Mongo is healthy on `127.0.0.1:27018`; application environment files were not changed. Acceptance cleanup explicitly enables the Compose acceptance profile and checks that the generated project's containers are gone; initial review fixtures were removed without touching development volumes or Supabase. No commit, push, deployment or remote migration was performed.

Start with S6 deferred outcomes and remaining race tests. Real Mongo store tests and isolated scheduler contention tests now exist; extend them instead of replacing them with mocks. S5 reservation/direct upload and S7 bridge are not implemented. The previous percentage estimates were not tied to weighted tasks and should not be repeated as measured completion. No subagents are needed for these packets.
...
Next implementation action: capture CI/migration replay evidence, then close the remaining L4/S4 Mongo acceptance and S5 storage acceptance before starting S7. The owner-scoped reviewer assignment feature is implemented locally: `PATCH /api/property-scans/[scanId]/reviewers` replaces assignments behind the current session revision, and Scan Studio can grant or revoke reviewer user IDs. The sprint workspace now presents conservative assignment states, exposes owner-controlled completion for assigned/in-progress work, preserves backlog descriptions/estimates on partial edits, permits revision-fenced edits to proposed sprint items with supported worker selection, keeps a proposed-item draft open after a failed or stale save, uses accurate Save schedule copy for paused schedules, the shortlist planner displays created-task counts plus actionable exclusions while labeling notes as You or Jamie, and Pulse command results can be added once to the owner backlog with command provenance. S6 now adds replay-safe event jobs and a bounded sprint-planner event producer contract; it does not register property reconstruction or auto-send. Focused feature tests pass 7 files / 38 tests across scheduler events/route/registry/policy, sprint selection/route/presentation; the local DB suite passes 44/44 scheduler assertions after a local migration reset; the two-session concurrency harness passes; `npm run build` passes with only the existing non-fatal Kepler dynamic-usage warning; and the authenticated clean-server browser boundary spec passes 2/2. CI migration replay, Mongo/storage acceptance, S7 job bridge, S8 processor and S9 publication remain gated. Do not spend the next sprint polishing the room-shell demo.
...
| Active / next packet | S6 event-job foundation, local 44/44 DB acceptance, and two-session concurrency acceptance are complete: recurring schedules and one-off events now share durable jobs, leases, retries and owner fencing. Next is CI/migration replay evidence plus L4 browser/Mongo acceptance before S7, without bypassing release gates |
| Code changes from this handoff | L1 boundary; L2 actor policy, reviewer IDs, stable asset IDs, actor-scoped lookup, private signed-URL route/gallery, owner-scoped reviewer grant/revoke route and Scan Studio controls; L3 saved-session recovery, camera attach/cleanup, upload retry state; L4 consent receipt, revision/hash checks, review events, asset dedupe/downstream invalidation, explicit server-enforced review acknowledgement, owner-scoped listing resolver, immutable mock DTO snapshots and race tests; S5 buffered upload validation, signature checks, full-batch prevalidation, cleanup and route tests; Scan Studio prefetch boundary and browser spec; sprint assignment state presentation/completion control, backlog edit preservation, proposed-item revision fence/worker selection, failed-save draft preservation and Pulse result-to-backlog provenance/deduplication; shortlist planning exclusion summary and explicit You/Jamie note labels; paused-schedule Save schedule label; S6 event-job migration, event contract registry, enqueue helper, sprint-planner event producer contract, claim-path support, workflow payload propagation, event provenance UI, and local two-session acceptance harness |
| Evidence carried forward | Combined scheduler/sprint suite: 7 files / 38 tests passed from `apps/pulse`; local `npm run test:db` passed 44/44 after `supabase db reset --local --no-seed --yes` applied migrations through `20260916020000_event_workflow_jobs.sql`; `npm run test:db:concurrency` passed with two independent claim sessions and duplicate event enqueue replaying one job; `npm run build` passed (Next compiled, type checked, generated 256 static pages, with only the existing non-fatal Kepler dynamic-usage warning); authenticated clean-server `npx playwright test tests/property-scan-boundaries.spec.ts` passed 2/2; the workspace-root npm wrapper hit a Windows/esbuild access-denied config-resolution error, so that malformed invocation is not counted as a product failure; earlier reviewer/sprint presentation suite: 2 files / 7 tests passed; earlier reviewer assignment/store/lifecycle suite: 3 files / 9 tests passed; `git diff --check` passed with line-ending normalization warnings only |
| Remaining blockers | CI migration replay and remote check evidence; L4 full acceptance: disposable Mongo concurrency, clean-server browser/deep-link assertions and stale-artifact browser evidence; S5 full media decoding/provenance, storage-policy integration, real limits, retry/orphan reconciliation and post-change build evidence; authenticated browser/mobile acceptance; denied/late permission and failed-upload/unmount coverage; actual processor unselected; no PR/CI/remote migration/deployment evidence in this turn |
| Changed paths / commit | Runtime, tests and plan docs changed locally; not committed |
| Exact next action | Capture CI/migration replay evidence for S6, then return to L4 browser/Mongo acceptance and S5 storage acceptance; only after those release gates pass begin S7's approved-scan outbox bridge |
