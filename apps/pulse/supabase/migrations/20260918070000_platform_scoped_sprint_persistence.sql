-- W2: persist a scheduled manual sprint under the schedule's workspace scope.
-- The leased job, schedule mapping, membership, selected backlog mappings and
-- sprint creation are checked/locked in one transaction.
...
BEGIN
  ...
  IF NOT FOUND OR locked_job.status <> 'running'
    OR locked_job.lease_token IS NULL
  ...
  IF NOT EXISTS (
    SELECT 1 FROM public.platform_memberships AS membership
  ...
  IF EXISTS (
    ...
    LEFT JOIN public.sprint_backlog_items AS backlog
      ON backlog.id = NULLIF(item->>'backlog_item_id', '')::UUID
     AND backlog.owner_id = locked_job.user_id
    LEFT JOIN public.platform_scope_links AS backlog_scope
      ON backlog_scope.resource_type = 'sprint_backlog_item'
     AND backlog_scope.resource_id = backlog.id::TEXT
     AND backlog_scope.workspace_id = p_workspace_id
     AND backlog_scope.status = 'mapped'
    WHERE backlog.id IS NULL OR backlog_scope.id IS NULL
  ) THEN
  ...
  SELECT created_sprint_id, locked_job.user_id, backlog.id,
    backlog.title, backlog.description, backlog.priority, backlog.estimate_minutes, 'proposed'
  FROM jsonb_array_elements(p_items) AS item
