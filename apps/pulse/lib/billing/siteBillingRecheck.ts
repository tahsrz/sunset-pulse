...
async function findStripeSubscriptionForKit(kit: AgentLaunchKit) {
  ...
  if (storedSubscriptionId) {
    ...
    if (isDeletedSubscription(subscription)) {
      throw new Error('The stored Stripe subscription no longer exists; manual billing re-link is required.');
    }
    return subscription as Stripe.Subscription;
  }

  throw new Error('This site does not have a Stripe subscription pointer; manual billing re-link is required.');
}
