...
export function formatHeadlessCatalog(host: string, cartridges: PulseCartridge[]) {
  ...
  return [
    ...
    ...metadata.flatMap((cartridge, index) => [
      ...
      `SHA256: ${cartridge.checksumSha256 || 'UNKNOWN'}`,
      ...(cartridge.payloadChecksumSha256 && cartridge.payloadChecksumSha256 !== cartridge.checksumSha256 ? [`PAYLOAD_SHA256: ${cartridge.payloadChecksumSha256}`] : []),
...
export function formatHeadlessCartridge(host: string, cartridge: PulseCartridge, previews: HeadlessPreview[]) {
  ...
  return [
    ...
    `SHA256: ${metadata.checksumSha256 || 'UNKNOWN'}`,
    ...(metadata.payloadChecksumSha256 && metadata.payloadChecksumSha256 !== metadata.checksumSha256 ? [`PAYLOAD_SHA256: ${metadata.payloadChecksumSha256}`] : []),
