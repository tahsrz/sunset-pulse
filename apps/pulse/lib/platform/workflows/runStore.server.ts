...
import { cancelRunSchema, checkpointResponseSchema, startRunSchema } from '@/lib/platform/contracts/run';
...
export async function listRuns(actorId: string, workspaceId: string) {
  await requireWorkspaceAccess(actorId, workspaceId, 'workspace:read');
  const { data, error } = await supabaseAdmin.from('platform_runs')
    .select('id,workspace_id,definition,definition_hash,state,status,revision,created_at,updated_at')
    .eq('workspace_id', workspaceId).order('created_at', { ascending: false }).limit(50);
  if (error) throw new PlatformRunError(error.code);
  return data || [];
}
export async function listCheckpoints(actorId: string, workspaceId: string) {
  await requireWorkspaceAccess(actorId, workspaceId, 'workspace:read');
  const { data, error } = await supabaseAdmin.from('platform_checkpoints')
    .select('id,run_id,node_id,type,prompt,response_schema,target,status,revision,created_at')
    .eq('workspace_id', workspaceId).eq('status', 'pending').order('created_at', { ascending: true }).limit(100);
  if (error) throw new PlatformRunError(error.code);
  return data || [];
}
