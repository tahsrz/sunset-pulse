...
import { recordEffectRecoveryReview } from '@/lib/platform/workflows/exceptionRecovery.server';
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  return workspaceWorkflowRequest(request, workspaceContext, async (actor, workspace) =>
    recordEffectRecoveryReview(actor, workspace, receiptId, await readWorkflowBody(request)));
}
