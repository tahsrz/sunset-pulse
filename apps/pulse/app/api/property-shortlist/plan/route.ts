...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const existingTasks = (existing || []).map((task) => ({ dedupeKey: task.dedupe_key, status: task.status }));
    const plan = buildPropertyBacklog({ properties: planningProperties, existingTasks });
