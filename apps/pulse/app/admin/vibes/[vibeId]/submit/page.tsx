...
export default function SubmitVibePage() {
  ...
  useEffect(() => { fetch(`/api/admin/vibes/${encodeURIComponent(vibeId)}`).then((r) => r.json()).then((p) => setVibe(p.vibe)).catch(() => setError('Unable to load vibe.')); }, [vibeId]);
  async function submit() { setSubmitting(true); const response = await fetch(`/api/admin/vibes/${encodeURIComponent(vibeId)}/submit`, { method: 'POST' }); const payload = await response.json(); if (!response.ok) { setError(payload.error || 'Submission failed.'); setSubmitting(false); return; } router.push(`/admin/vibes/${encodeURIComponent(vibeId)}/edit`); }
