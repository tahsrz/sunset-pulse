...
export function TaxonomyDirectory() {
  ...
  return (
    <section className="border border-slate-200 bg-white" aria-label="Vibe taxonomy directory">
      ...
      {manageTerms && taxonomyGroupRows.length > 0 ? <div className="border-b border-slate-200"><div className="flex items-center justify-between gap-3 bg-slate-50 px-4 py-3"><h2 className="text-sm font-bold text-slate-900">Taxonomies</h2><span className="text-xs text-slate-500">{taxonomyGroupRows.length} groups</span></div><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="border-y border-slate-200 bg-white text-xs font-semibold uppercase tracking-wide text-slate-500"><tr><th scope="col" className="px-4 py-2">Name</th><th scope="col" className="px-4 py-2">Slug</th><th scope="col" className="px-4 py-2">Type</th><th scope="col" className="px-4 py-2 text-right">Active</th><th scope="col" className="px-4 py-2 text-right">Archived</th><th scope="col" className="px-4 py-2 text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-100">{taxonomyGroupRows.map((taxonomyGroup) => <tr key={taxonomyGroup.slug}><th scope="row" className="px-4 py-2">{editingGroupSlug === taxonomyGroup.slug ? <input aria-label={`Name for taxonomy ${taxonomyGroup.slug}`} required value={editingGroupLabel} onChange={(event) => setEditingGroupLabel(event.target.value)} className="w-full rounded border border-slate-300 px-2 py-1 text-sm" /> : <button type="button" onClick={() => setGroup(taxonomyGroup.slug)} className="font-semibold text-[#2271b1] hover:underline">{taxonomyGroup.label}</button>}</th><td className="px-4 py-2 font-mono text-xs text-slate-600">{taxonomyGroup.slug}</td><td className="px-4 py-2 text-slate-600">{taxonomyGroup.hierarchical ? 'Hierarchical' : 'Flat'}</td><td className="px-4 py-2 text-right font-semibold">{taxonomyGroup.activeTerms}</td><td className="px-4 py-2 text-right font-semibold">{taxonomyGroup.archivedTerms}</td><td className="px-4 py-2 text-right">{editingGroupSlug === taxonomyGroup.slug ? <span className="inline-flex gap-2"><button type="button" disabled={updating || !editingGroupLabel.trim()} onClick={() => void updateTaxonomyGroupLabel()} className="font-semibold text-[#2271b1] disabled:opacity-50">Save</button><button type="button" disabled={updating} onClick={() => { setEditingGroupSlug(''); setEditingGroupLabel(''); }} className="text-slate-500">Cancel</button></span> : <button type="button" onClick={() => { setEditingGroupSlug(taxonomyGroup.slug); setEditingGroupLabel(taxonomyGroup.label); }} className="font-semibold text-[#2271b1]">Edit</button>}</td></tr>)}</tbody></table></div></div> : null}
