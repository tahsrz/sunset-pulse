...
export function VibeConfirmDialog({ open, title, description, confirmLabel, cancelLabel, busy = false, onConfirm, onOpenChange }: VibeConfirmDialogProps) {
  ...
  return <dialog ref={dialogRef} aria-labelledby={titleId} onCancel={(event) => { event.preventDefault(); close(); }} onClose={() => { if (!busy) triggerRef.current?.focus(); }} className="w-[min(32rem,calc(100vw-2rem))] rounded border border-slate-300 p-0 shadow-xl backdrop:bg-slate-900/50">
    <form method="dialog" onSubmit={(event) => event.preventDefault()} className="p-5">
      ...
      <div className="mt-5 flex justify-end gap-2">
        ...
        <button type="button" disabled={busy} onClick={confirm} className="rounded bg-[#2271b1] px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">{busy ? 'Working…' : confirmLabel}</button>
      </div>
    </form>
  </dialog>;
}
