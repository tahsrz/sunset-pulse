...
export default async function VibeSourcePage({ params }: { params: Promise<{ vibeId: string }> }) {
  ...
  return <main className="min-h-screen bg-slate-100 px-4 py-8 text-slate-900 sm:px-8"><div className="mx-auto max-w-3xl"><VibePageHeader title="Source media" description={<>Provenance for {vibe.title || vibe.name || vibe.vibeId}.</>} backHref={`/vibes/${vibeId}/edit`} backLabel="Back to editor" /><section className="mt-6 border border-slate-200 bg-white p-5 sm:p-6"><dl className="space-y-5 text-sm"><div><dt className="font-bold uppercase text-slate-400">Kind</dt><dd className="mt-1">{source.kind || 'manual'}</dd></div><div><dt className="font-bold uppercase text-slate-400">URL or path</dt><dd className="mt-1 break-all font-mono text-xs">{source.url || source.path || 'Not recorded'}</dd></div><div><dt className="font-bold uppercase text-slate-400">Attribution</dt><dd className="mt-1">{source.attribution || 'Not recorded'}</dd></div><div><dt className="font-bold uppercase text-slate-400">Ownership note</dt><dd className="mt-1">{source.ownershipNote || 'Not recorded'}</dd></div></dl></section></div></main>;
}
