...
export default function ApplyVibePage() {
  ...
  async function apply() {
    ...
    try {
      const response = await fetch(
        `/api/admin/sites/${encodeURIComponent(siteId)}/apply-vibe?tenantId=default`,
        {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ vibeId, revisionId }),
        },
      );
      const payload = await response.json();
      if (!response.ok)
        throw new Error(payload.error || "Unable to apply revision.");
      setMessage(`Revision ${revisionId} applied to site ${siteId}.`);
      await checkPointer();
    } catch (error) {
      setMessage(
        ...
        >
          <div className="rounded-lg border border-amber-200 bg-amber-50 p-4">
            <label className="block text-sm font-bold">
              Disposable verification run ID
              <input
                value={runId}
                onChange={(event) => setRunId(event.target.value)}
                placeholder="run-20260831"
                className="mt-2 w-full rounded-md border border-amber-300 bg-white px-3 py-2 font-mono font-normal"
              />
            </label>
            <button
              type="button"
          ...
          {pointer && (
            <p className="rounded-lg bg-slate-50 p-3 text-xs">
              ...
              <span className="font-mono">
                {pointer.revision?.revisionId || "none"}
              </span>
              {pointer.revision?.revisionNumber
                ? ` (revision ${pointer.revision.revisionNumber})`
                : ""}
              ; applied{" "}
              {pointer.appliedAt
                ? new Date(pointer.appliedAt).toLocaleString()
                : "never"}{" "}
              by {pointer.appliedBy || "unknown"}.
            </p>
          )}
          <label className="block text-sm font-bold">
            ...
            <input
              ...
              onChange={(event) => {
                ...
                setRevisionId(value);
                if (value !== linkedRevisionId) setRevisionNumber("");
              }}
          ...
          {revisionNumber ? (
            <p className="rounded-lg bg-sky-50 p-3 text-sm text-sky-950">
              Selected from revision history: <strong>r{revisionNumber}</strong>{" "}
              · <span className="font-mono text-xs">{revisionId}</span>
            </p>
          ) : null}
          {siteId && revisionId ? (
            <dl className="grid gap-2 rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm">
              <div>
                <dt className="font-bold">Vibe</dt>
                <dd className="font-mono text-xs">{vibeId}</dd>
              </div>
              <div>
                <dt className="font-bold">Site</dt>
                <dd className="font-mono text-xs">{siteId}</dd>
              </div>
              <div>
                <dt className="font-bold">Current pointer</dt>
                <dd className="font-mono text-xs">
                  {pointer?.revision?.revisionId || "not checked"}
                </dd>
              </div>
              <div>
                <dt className="font-bold">New pointer</dt>
                <dd className="font-mono text-xs">{revisionId}</dd>
              </div>
              {siteId.startsWith("cms-verification-") ? (
                <p className="font-semibold text-amber-800">
                  Disposable verification site: this site is temporary and
          ...
          <p className="rounded-lg bg-amber-50 p-3 text-sm text-amber-900">
            Only apply a published revision belonging to this Vibe. The
            protected site API verifies this again before it changes a live
            pointer.
          </p>
          {message && (
