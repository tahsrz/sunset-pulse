...
export default function ApplyVibePage() {
  ...
  return <main className="min-h-screen bg-slate-100 px-4 py-8 text-slate-900 sm:px-8"><div className="mx-auto max-w-2xl"><Link href={`/vibes/${vibeId}/revisions`} className="text-sm font-semibold text-slate-500 hover:underline">← Back to revisions</Link><h1 className="mt-4 text-3xl font-black">Apply published revision</h1><p className="mt-1 text-sm text-slate-600">Vibe <span className="font-mono">{vibeId}</span> · confirm the exact published revision before applying.</p><form onSubmit={apply} className="mt-6 space-y-5 rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><div className="rounded-lg border border-amber-200 bg-amber-50 p-4"><label className="block text-sm font-bold">Disposable verification run ID<input value={runId} onChange={event => setRunId(event.target.value)} placeholder="run-20260831" className="mt-2 w-full rounded-md border border-amber-300 bg-white px-3 py-2 font-mono font-normal" /></label><button type="button" onClick={() => setSiteId(`cms-verification-${runId.trim()}`)} disabled={!/^[a-z0-9-]{6,64}$/.test(runId.trim())} className="mt-3 rounded-lg border border-amber-400 px-3 py-2 text-sm font-semibold disabled:opacity-50">Use disposable site</button></div><label className="block text-sm font-bold">Site ID<input required value={siteId} onChange={event => setSiteId(event.target.value)} className="mt-2 w-full rounded-md border border-slate-300 px-3 py-2 font-normal" /></label><button type="button" onClick={() => void checkPointer()} disabled={!siteId || busy} className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold">Check current site pointer</button>{pointer && <p className="rounded-lg bg-slate-50 p-3 text-xs">Current pointer: <span className="font-mono">{pointer.revision?.revisionId || 'none'}</span>{pointer.revision?.revisionNumber ? ` (revision ${pointer.revision.revisionNumber})` : ''}; applied {pointer.appliedAt ? new Date(pointer.appliedAt).toLocaleString() : 'never'} by {pointer.appliedBy || 'unknown'}.</p>}<label className="block text-sm font-bold">Published revision ID<input required value={revisionId} onChange={event => { const value = event.target.value; setRevisionId(value); if (value !== linkedRevisionId) setRevisionNumber(''); }} className="mt-2 w-full rounded-md border border-slate-300 px-3 py-2 font-mono font-normal" /></label>{revisionNumber ? <p className="rounded-lg bg-sky-50 p-3 text-sm text-sky-950">Selected from revision history: <strong>r{revisionNumber}</strong> · <span className="font-mono text-xs">{revisionId}</span></p> : null}{siteId && revisionId ? <dl className="grid gap-2 rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm"><div><dt className="font-bold">Vibe</dt><dd className="font-mono text-xs">{vibeId}</dd></div><div><dt className="font-bold">Site</dt><dd className="font-mono text-xs">{siteId}</dd></div><div><dt className="font-bold">Current pointer</dt><dd className="font-mono text-xs">{pointer?.revision?.revisionId || 'not checked'}</dd></div><div><dt className="font-bold">New pointer</dt><dd className="font-mono text-xs">{revisionId}</dd></div>{siteId.startsWith('cms-verification-') ? <p className="font-semibold text-amber-800">Disposable verification site: this site is temporary and subject to automatic expiry.</p> : null}</dl> : null}<p className="rounded-lg bg-amber-50 p-3 text-sm text-amber-900">Only apply a published revision belonging to this Vibe. The protected site API verifies this again before it changes a live pointer.</p>{message && <p role="status" className="text-sm text-slate-700">{message}</p>}<button disabled={busy} className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-bold text-white disabled:opacity-50">{busy ? 'Applying…' : 'Apply revision'}</button></form></div></main>;
}
