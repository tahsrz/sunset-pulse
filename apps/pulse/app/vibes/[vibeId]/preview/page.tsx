...
export default async function VibePreviewPage({ params }: { params: Promise<{ vibeId: string }> }) {
  ...
  return <main className="min-h-screen px-4 py-8 sm:px-8" style={{ background, color: text }}><div className="mx-auto max-w-5xl"><Link href={`/vibes/${vibeId}/edit`} className="text-sm font-semibold opacity-70 hover:underline">← Back to editor</Link><h1 className="mt-4 text-3xl font-black">{data.vibe.title} preview</h1><p className="mt-1 text-sm opacity-70">Draft preview · not published</p><section className="mt-8" style={sectionStyle}><h2 style={{ fontFamily: typography.fontFamilyHeading, fontSize: `${typography.scaleRatio * 2.25}rem`, fontWeight: typography.fontWeightBold }}>A clear next step for every visitor.</h2><p className="mt-4 max-w-xl opacity-80">This representative layout renders the saved draft colors, typography, and layout tokens. It remains a draft-only preview until a published revision is explicitly applied to a site.</p><button className="mt-8 px-4 py-2 text-sm" style={{ background: accent, borderRadius: radii[layout.borderRadius], color: text, fontFamily: typography.fontFamilyBody, fontWeight: typography.fontWeightBold }}>Ask Jamie</button></section></div></main>;
}
