...
import { propertyScanModes, scanModeCopy, type PropertyScanMode } from '@/lib/scans/propertyScanContract';
...
type ScanSession = {
  ...
  status: string;
  assets: Array<{ fileName: string; size: number; mimeType: string }>;
};
...
export default function ScanStudioPage() {
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      setMessage('Camera access was not granted. You can still upload captures from the device.');
    }
  };
    ...
    <main className="min-h-screen bg-[#07131f] text-white">
      <section className="border-b border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(20,184,166,0.2),transparent_38%),linear-gradient(135deg,#0a1c2d,#07131f)] px-6 py-14">
        <div className="mx-auto max-w-6xl">
          <Link href="/properties" className="text-xs font-black uppercase tracking-[0.25em] text-teal-200 hover:text-white">← Back to properties</Link>
      ...
      <section className="mx-auto grid max-w-6xl gap-6 px-6 py-10 lg:grid-cols-[1fr_0.9fr]">
        <div className="rounded-3xl border border-white/10 bg-white/[0.05] p-6 md:p-8">
          ...
          <div className="mt-6"><p className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">Capture method</p><div className="mt-3 grid gap-3 sm:grid-cols-3">{propertyScanModes.map((scanMode) => <button key={scanMode} type="button" onClick={() => setMode(scanMode)} className={`rounded-2xl border p-4 text-left transition ${mode === scanMode ? 'border-teal-200/70 bg-teal-200/15' : 'border-white/10 bg-slate-950/40 hover:border-white/25'}`}><span className="block text-sm font-black text-white">{scanModeCopy[scanMode].label}</span><span className="mt-2 block text-xs leading-5 text-slate-400">{scanModeCopy[scanMode].detail}</span></button>)}</div></div>
        ...
        <div className="space-y-6">
          <div className="rounded-3xl border border-white/10 bg-white/[0.05] p-6"><div className="flex items-center gap-3"><Camera className="h-5 w-5 text-teal-200" /><h2 className="font-black uppercase tracking-tight">Capture checklist</h2></div><ul className="mt-5 space-y-3">{selectedMode.instructions.map((instruction) => <li key={instruction} className="flex gap-3 text-sm leading-6 text-slate-300"><CheckCircle2 className="mt-1 h-4 w-4 shrink-0 text-teal-200" />{instruction}</li>)}</ul></div>
          {session ? <div className="rounded-3xl border border-teal-200/20 bg-teal-200/[0.06] p-6"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.25em] text-teal-200">Session {session.scanId.slice(-8)}</p><h2 className="mt-2 text-xl font-black uppercase">{session.status.replace('_', ' ')}</h2></div><CloudUpload className="h-7 w-7 text-teal-200" /></div><div className="mt-5 flex flex-wrap gap-3"><button type="button" onClick={cameraOn ? stopCamera : () => void startCamera()} className="inline-flex items-center gap-2 rounded-full border border-white/15 px-4 py-2 text-xs font-black uppercase tracking-widest hover:border-teal-200/60"> <Camera className="h-4 w-4" />{cameraOn ? 'Stop camera' : 'Preview camera'}</button><label className="inline-flex cursor-pointer items-center gap-2 rounded-full bg-white px-4 py-2 text-xs font-black uppercase tracking-widest text-slate-950 hover:bg-teal-100"><Upload className="h-4 w-4" />Upload captures<input type="file" className="hidden" accept="image/jpeg,image/png,image/webp,video/mp4,video/quicktime" capture="environment" multiple onChange={(event) => void uploadCaptures(event.target.files)} /></label></div>{cameraOn && <video ref={videoRef} autoPlay muted playsInline className="mt-5 aspect-video w-full rounded-2xl border border-white/10 bg-black object-cover" />}{session.assets.length > 0 && <p className="mt-5 text-sm text-teal-100">{session.assets.length} private capture{session.assets.length === 1 ? '' : 's'} ready for agent review.</p>}</div> : <div className="rounded-3xl border border-dashed border-white/15 p-6 text-sm leading-7 text-slate-400">Create a session to unlock camera preview and private upload. No capture is sent before you approve the consent checkboxes.</div>}
        </div>
      </section>
    </main>
  );
