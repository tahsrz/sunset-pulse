...
export function RunDetail({ workspaceId, runId }: { workspaceId: string; runId: string }) {
  ...
  if (error) return <main className="min-h-screen bg-slate-950 p-8 text-rose-100"><p role="alert">{error}</p></main>;
  if (!payload) return <main className="min-h-screen bg-slate-950 p-8 text-slate-300">Loading run…</main>;
  return <main className="min-h-screen bg-slate-950 px-5 py-10 text-white sm:px-10"><div className="mx-auto max-w-3xl space-y-6">
    <Link href={`/workspaces/${workspaceId}/inbox`} className="text-sm text-cyan-200">← Back to inbox</Link>
