...
export function SpawnAgentDialog({ open, workers, onClose, onSpawn }: { open: boolean; workers: IntelligenceWorker[]; onClose: () => void; onSpawn: (input: { workerId: string; label?: string; assignment?: string; autoListenEnabled?: boolean }) => void }) {
  ...
  useEffect(() => {
    ...
    const controls = () => Array.from(container?.querySelectorAll<HTMLElement>('button:not(:disabled), input, select, textarea') || []);
  ...
  return <div ref={dialog} className="fixed inset-0 z-50 grid place-items-center bg-slate-950/80 p-4" role="dialog" aria-modal="true" aria-labelledby="spawn-agent-title"><form onSubmit={(event) => { event.preventDefault(); onSpawn({ workerId, label, assignment, autoListenEnabled: listen }); setLabel(''); setAssignment(''); setListen(false); onClose(); }} className="w-full max-w-lg rounded-lg border border-cyan-200/30 bg-[#0c1b25] p-5 text-white shadow-2xl">
