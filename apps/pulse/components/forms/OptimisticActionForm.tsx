...
export function OptimisticActionForm({
  ...
  const [state, formAction, isActionPending] = useActionState(action, { ...initialState, message: idleMessage });
  ...
  return (
    <form action={submitWithOptimism} className={className}>
      {children({ ...optimisticState, status: isActionPending ? 'pending' : optimisticState.status })}
    </form>
  );
}
...
export function OptimisticSubmitButton({
  ...
  const isPending = optimisticState.status === 'pending';
