...
/**
 * Memoria-Synchronized Supabase Client
 * v3.5 Architecture: Ensures consistent URL resolution across Vercel/Local boundaries.
 */
...
const getSupabaseConfig = () => {
  ...
  if (typeof window !== 'undefined' && (!url || url.includes('vercel.app'))) {
    console.warn('⚠️ [SUPABASE_ROBUSTNESS]: NEXT_PUBLIC_SUPABASE_URL is missing or misconfigured. Falling back to hardcoded project URL.');
    url = 'https://xlyfhiafactxahhvikyv.supabase.co'; 
  }

  return {
    url: url || 'https://xlyfhiafactxahhvikyv.supabase.co', // Fail-safe to project ID identified in .env
    key: key || 'placeholder-key',
    serviceRole: process.env.SUPABASE_SERVICE_ROLE_KEY
  };
};

const { url: supabaseUrl, key: supabaseKey, serviceRole: serviceRoleKey } = getSupabaseConfig();
...
export const toggleCollection = async (userId, propertyId) => {
  if (typeof window !== 'undefined') {
    // Client-side execution: reroute through the backend API proxy to ensure App Router cache invalidation
    ...
    if (!res.ok) {
      throw new Error('Failed to toggle collection through backend proxy');
    }
    return res.json();
  } else {
