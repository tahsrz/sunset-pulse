...
// The Listing Gate: Probabilistic filter for surgical synchronization
const GATE_PATH = path.resolve(process.cwd(), 'cartridges/listings_gate.tah');
const listingGate = fs.existsSync(GATE_PATH) ? new TAHGate(GATE_PATH) : null;
...
export const syncPropertyToIntelligenceGrid = async (propertyData) => {
  ...
  if (listingGate && mlsId) {
    const signature = `${mlsId}_${lastUpdated}`;
    if (listingGate.isProbablyPresent(signature)) {
