...
export const POST = async (request) => {
  try {
    ...
    const { userId, user } = sessionUser; // Assuming sessionUser has email/name or we need to fetch user
    ...
    // CRM INTELLIGENCE: Auto-capture Lead upon booking
    const property = await Property.findById(propertyId);
    const isRV = property?.type === 'RV' || property?.type === 'RV Park';
    
    // Attempt to find or create lead
    // We use the email from the session if available, otherwise we might need the user model
    // For now, let's assume we can get user details
    const userEmail = user?.email || sessionUser.email;
    const userName = user?.name || sessionUser.name || 'Valued Client';
